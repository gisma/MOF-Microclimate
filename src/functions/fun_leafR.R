...
#' @param matrixLAD  if TRUE returns matrix of LAD profiles
#' @param meanLAD if TRUE returns average LAD of the scene
#' @return A single column [`data.frame`][base::data.frame] of the 3D voxels information (xyz) with Leaf Area Density values
#'         A matrix [`data.frame`][base::data.frame] of the 3D voxels information (xyz) with Leaf Area Density values for each voxel
#' @param maxP treshold for maximum height of LAD profile
#' 
...
#' @export
lad.voxels = function(normlas.file, grain.size = 1, k = 1, maxP=45,meanLAD=FALSE, matrixLAD = TRUE){
  ...
  if (maxZ>maxP) {
    maxZ =maxP
    .las@data$Z[.las@data$Z > maxP] = maxP
    }
  ...
  #e <- ext(t.binneds)
  #tt.binneds = terra::crop(t.binneds,e,mask = T) 
  p <- terra::as.points(t.binneds)
  t.binneds <- data.frame(terra::geom(p),terra::values(p))
  #t.binneds2    = data.frame(crds(tt.binneds), terra::values(tt.binneds))
  t.binneds = t.binneds[,-c(1,2,5)]
  ...
  #pulses.profile.dz1 = cbind(ground.returns,pulses.profile.dz1 )
  rm(ground.returns)
  ...
  # lidR function  
  # f = function(x){
  #   lidR::LAD(t(x),dz = dz, z0 = htmin, k = k)
  # }
  # 
  # apply(pulses.profile.dz1, 1,f )
  # 
  ...
  # remove all NA 
  #LAD.dz1 = LAD.dz1[rowSums(is.na(LAD.dz1)) == 0,]
  
  
  #fuel list object
  LAD_VOXELS[["LAD"]] = LAD.dz1
  LAD_VOXELS[["coordenates"]] = t.binneds[,c("X", "Y")]
  LAD_VOXEL_DF = data.frame(LAD=LAD.dz1, X =t.binneds[,"X"],Y=t.binneds[,"Y"])
  
  
  names(LAD_VOXEL_DF) = gsub(c(rev(names(t.binneds[4:(length(t.binneds))])),"X","Y"),pattern = "pulses",replacement = "LAD")
  
  LAD_VOXEL_DF[is.na(LAD_VOXEL_DF)] <- 0.0
  #invert data.frames for the sky be first
  LAD_VOXEL_DF = LAD_VOXEL_DF[,length(LAD_VOXEL_DF):1] #invert columns
  
  
  rm(LAD.dz1, t.binneds)
  if (meanLAD) return(LAD_VOXELS)
  else if (matrixLAD) return(LAD_VOXEL_DF )
}#End function
...
lad.profile = function(VOXELS_LAD, relative = FALSE){
  VOXELS_LAD[[1]]
  t.lad.profile2 = lapply(VOXELS_LAD[[1]], mean , na.rm = TRUE)
  data.frame(LAD=VOXELS_LAD$LAD[1:23],X=VOXELS_LAD$coordenates[1],Y=VOXELS_LAD$coordenates[2])
  if(relative == TRUE){
